...
public:
	...
	HD_GrowingArray();
	HD_GrowingArray(SizeType aCapacity);
	...
	void Remove(u32 aIndex);
	void RemoveCyclic(u32 aIndex);
	void RemoveAll();
...
private:
	static constexpr f32 ourGrowFactor = 1.5;

	void CheckSizeAndGrowIfNecessary();
	void Grow(SizeType aNewCapacity);
	void InsertAtIndex(const T& aItem, u32 aIndex);
...
template<typename T>
HD_GrowingArray<T>::HD_GrowingArray(SizeType aCapacity)
	: mySize(0)
	...
	, myCapacity(0)
{
	...
	for (u32 i = 0; i < aGrowingArray.GetSize(); i++)
	{
...
HD_GrowingArray<T>::HD_GrowingArray(std::initializer_list<T> aInitializerList)
	...
	, myCapacity(0)
{
	SizeType initializerListSize = static_cast<SizeType>(aInitializerList.size());
	Reserve(initializerListSize);

	for (u32 i = 0; i < initializerListSize; i++)
	{
...
template<typename T>
void HD_GrowingArray<T>::Remove(u32 aIndex)
{
	assert(!GetIsEmpty() && 0 <= aIndex && aIndex < mySize);

	if (aIndex < mySize - 1)
	{
		for (u32 i = aIndex; i < mySize - 1; i++)
...
template<typename T>
void HD_GrowingArray<T>::RemoveCyclic(u32 aIndex)
{
	assert(!GetIsEmpty() && 0 <= aIndex && aIndex < mySize);
...
template<typename T>
SizeType HD_GrowingArray<T>::GetSize() const
{
	return mySize;
}
...
template<typename T>
void HD_GrowingArray<T>::Reserve(SizeType aCapacity)
{
...
template<typename T>
void HD_GrowingArray<T>::Resize(SizeType aSize)
{
	if (aSize <= mySize)
		return;
...
{
	T* tempData = myData;
	SizeType tempSize = mySize;
	SizeType tempCapacity = myCapacity;
...
template<typename T>
T& HD_GrowingArray<T>::operator[](u32 aIndex)
{
...
template<typename T>
const T& HD_GrowingArray<T>::operator[](u32 aIndex) const
{
	assert(0 <= aIndex && aIndex < mySize);
	return myData[aIndex];
}
...
void HD_GrowingArray<T>::CheckSizeAndGrowIfNecessary()
{
	...
	{
		SizeType newCapacity = myCapacity == 0 ? 2 : static_cast<SizeType>(myCapacity * ourGrowFactor);
		Grow(newCapacity);
	}
}

template<typename T>
void HD_GrowingArray<T>::Grow(SizeType aNewCapacity)
{
	T* oldData = myData;
...
template<typename T>
void HD_GrowingArray<T>::InsertAtIndex(const T& aItem, u32 aIndex)
{
	CheckSizeAndGrowIfNecessary();
	mySize++;

	for (s32 i = mySize - 1; static_cast<u32>(i) > aIndex; i--)
	{
		myData[i] = HD_Move(myData[i - 1]);
	}
